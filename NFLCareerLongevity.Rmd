...
It has a recall of 0.1, which is very low, meaning that from all observations, the model only correctly identified those players that had >= 3 years left 10% of the time.

It has an F1 score of 0.17, which is a very low balanced metric of recall and precision indicating this is probably not a good model, so instead we will try a model with interaction
...
In conclusion, it proved to be harder than we initially thought to predict retirement with great accuracy. Although our models had ~60% accuracy, they had low F1 and recall scores indicating they were not that good at identifying players that had longer left in their careers. However, as our project we were looking at retirement so it is good to note that our models were much better at identifying those players that were close to retirement. This would be useful for coaches as if a player flags that they have less time left in the NFL, coaches could either take measures to help prolong that player's career such as minute limits, or know that they have to start searching for a replacement.
