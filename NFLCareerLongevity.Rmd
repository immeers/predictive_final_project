...
```{r Random Effects}
library(lme4)
pos_data <- final %>% ungroup() %>% select(Season, Age, Pos, G, GS, AllTD, Pts, TotalInjuryCount, ConcussionTotal, YearsRemaining)
position <- lmer(YearsRemaining ~ . + (1|Pos), data = pos_data)
summary(position)
